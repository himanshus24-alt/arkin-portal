...
# =============================================================================
# CSS — ALL in st.markdown so it reaches the real DOM (st.html is shadow DOM)
# =============================================================================
def inject_css():
    st.markdown("""
<style>
...
:root {
    ...
    --ink1:   #0F3D3E; --ink2:   #2D5A5B; --ink3:   #6B8788;
    --line:   #E5EAEA;
...
/* Make containers transparent so gradient shows through */
.block-container,
...
/* ══ BRAND BAR — col1 ═══════════════════════════════════════════════════════
   The brand bar is rendered via st.markdown inside a column.
   col2 (sign-out) is given the same dark background so they look unified.   */
.brand-bar {
    background: var(--teal);
    color: #fff;
    padding: 14px 16px;
    border-radius: 14px 0 0 14px;   /* right corners handled by col2 CSS */
    position: relative;
    overflow: hidden;
    height: 100%;
...
/* Kill any white-on-white — force dark text on all select descendants */
[data-testid="stSelectbox"] * {
...
[data-testid="stSelectbox"] label {
    ...
    font-weight: 600 !important;
    color: #2D5A5B !important;
    text-transform: uppercase;
    letter-spacing: .5px;
}

/* ══ KPI TILES ══════════════════════════════════════════════════════════════ */
.kpi-card  { background:rgba(255,255,255,0.82); backdrop-filter:blur(10px); -webkit-backdrop-filter:blur(10px); border-radius:12px; padding:12px 13px; border:1px solid rgba(255,255,255,0.6); box-shadow:0 2px 12px rgba(15,61,62,0.10); margin-bottom:8px; }
...
/* ══ INFO CARDS ══════════════════════════════════════════════════════════════ */
.nudge-card    { background:rgba(253,244,237,0.88); backdrop-filter:blur(8px); -webkit-backdrop-filter:blur(8px); border-left:3px solid var(--copper); padding:12px 14px; border-radius:8px; margin-bottom:8px; font-size:13px; line-height:1.5; color:#6B3F1F; }
.announce-card { background:rgba(230,244,242,0.88); backdrop-filter:blur(8px); -webkit-backdrop-filter:blur(8px); border-left:3px solid var(--teal2); padding:12px 14px; border-radius:8px; margin-bottom:8px; font-size:13px; line-height:1.5; color:var(--teal); }
.action-card   { background:rgba(253,236,234,0.88); backdrop-filter:blur(8px); -webkit-backdrop-filter:blur(8px); border-left:3px solid #C8553D; padding:12px 14px; border-radius:8px; margin-bottom:8px; font-size:13px; line-height:1.5; color:#7A1F0E; }
.esc-card      { background:rgba(255,255,255,0.82); backdrop-filter:blur(8px); -webkit-backdrop-filter:blur(8px); border-radius:10px; border:1px solid rgba(255,255,255,0.6); padding:10px 12px; margin-bottom:8px; box-shadow:0 2px 12px rgba(15,61,62,0.10); }
...
/* ══ DEFAULT STREAMLIT BUTTONS (teal) ════════════════════════════════════════ */
.stButton button {
...
.stTextInput input {
    ...
    padding:10px 14px !important; background:#fff !important; color:var(--ink1) !important;
}
...
/* ══ TABS ════════════════════════════════════════════════════════════════════ */
.stTabs [data-baseweb="tab-list"] { gap:4px; background:rgba(255,255,255,0.80); backdrop-filter:blur(10px); -webkit-backdrop-filter:blur(10px); border-radius:10px; padding:4px; border:1px solid rgba(255,255,255,0.55); }
...
/* ══ RADIO ═══════════════════════════════════════════════════════════════════ */
.stRadio > div { gap:4px !important; background:rgba(255,255,255,0.80); backdrop-filter:blur(8px); -webkit-backdrop-filter:blur(8px); padding:4px; border-radius:10px; border:1px solid rgba(255,255,255,0.55); display:flex !important; }
...
.login-inner { position:relative; z-index:5; padding:96px 22px 0; max-width:440px; margin:0 auto; }
...
def build_escalation_table(perf_all, user, prod_ov=None):
    ...
    if df.empty or "Month Label" not in df.columns: return pd.DataFrame()
    ...
    return esc.sort_values("Streak", ascending=False) if not esc.empty else esc
...
def esc_card(rm_name, status, kind, visible_to, month_trail):
    chip = f'<span class="chip-{kind}">{status}</span>'
...
def rm_dashboard(user, data):
    ...
    if perf_m.empty: st.warning("No data for this month."); return
    row = perf_m.iloc[0]
    ...
    with t1:
        ...
        if not notif_m.empty:
            ...
            for i in range(1,6):
                ...
                if n.strip() and n!="nan": nudge_card(n)
        sec("Announcements","📢")
        ...
        if row["Actual Disb #"]==0: action_card("🚨 Zero disbursements this month — urgently review pipeline.")
        elif gap>0: action_card(f"🔥 Close {gap} more disbursements to hit monthly target.")
    ...
    with t2:
        ...
        fig=go.Figure(go.Funnel(y=["Logins","Sanctions","Disb"],
        ...
        c1,c2=st.columns(2)
        with c1:
            kpi("Conversion",f"{row['Actual Conversion %']:.1f}%",f"Tgt {row['Target Conversion %']:.1f}%",
                trend=row["Actual Conversion %"]-row["Target Conversion %"])
        ...
        pa=scope(data["perf"],user)
        if not pa.empty and "Month Label" in pa.columns:
            tr=pa.groupby("Month Label").agg(Disb=("Actual Disb #","sum"),Target=("Target Disb #","mean")).reindex(MONTH_ORDER).reset_index()
            fig=go.Figure()
            fig.add_bar(x=tr["Month Label"],y=tr["Disb"],name="Actual",marker_color="#1FA89A")
            fig.add_scatter(x=tr["Month Label"],y=tr["Target"],name="Target",mode="lines+markers",
...
def leader_dashboard(user, data, prod_ov=None):
    ...
    if perf_m.empty: st.warning("No team data for this month."); return
    ...
    with t1:
        ...
        if esc.empty:
            ...
            n_amber=len(esc[esc["Kind"]=="amber"]); n_red=len(esc[esc["Kind"]=="red"])
            c1,c2=st.columns(2)
        ...
        if low.empty: st.success("✅ No low performers this month.")
        else: st.dataframe(low.rename(columns={"EMP Name":"RM"}),use_container_width=True,hide_index=True)
        sec("Quick Nudge","✉️")
    ...
    with t2:
        sec("Team Snapshot","👥")
        total_disb=int(perf_m["Actual Disb #"].sum()); total_tgt=int(perf_m["Target Disb #"].sum())
        c1,c2=st.columns(2)
        with c1:
        ...
        ].rename(columns={"EMP Name":"RM","Actual Disb #":"Disb","Target Disb #":"Tgt",
                           "Actual Conversion %":"Conv%","IRR Mix %":"IRR%"})
        ...
        pa=scope(data["perf"],user,prod_ov)
        if not pa.empty and "Month Label" in pa.columns:
            tg=pa.groupby(["EMP Name","Month Label"])["Actual Disb #"].sum().reset_index()
            ...
            piv["Total"]=piv.sum(axis=1); piv["Avg/Mo"]=(piv["Total"]/max(1,len(piv.columns)-1)).round(1)
        ...
        if user["level"] in ("RBM","ZH","CXO"):
            ...
            st.write("**Top 5**"); st.dataframe(abm_p.nlargest(5,"Ach %"),use_container_width=True,hide_index=True)
            st.write("**Bottom 5**"); st.dataframe(abm_p.nsmallest(5,"Ach %"),use_container_width=True,hide_index=True)
...
def admin_dashboard(user, data):
    ...
    with t1:
        ...
        if up:
            ...
            st.cache_data.clear(); st.success("✅ Uploaded and cache cleared.")
        if DATA_FILE.exists():
            sz=DATA_FILE.stat().st_size/1024; mt=datetime.fromtimestamp(DATA_FILE.stat().st_mtime)
            st.info(f"📁 {DATA_FILE.name} · {sz:.1f} KB · {mt:%d-%b-%Y %H:%M}")
    with t2:
        notif=data["notif"]; cols=["Month Label","RM Name","Product","Notification 1"]
...
def login_screen():
    ...
    with st.form("arkin_login"):
        ...
        if st.form_submit_button("Sign in →",use_container_width=True):
            ...
            else: st.error("❌ Invalid Employee ID or password.")
    st.html("""
...
def main():
    ...
    # Sign out: brand bar button navigates to ?logout=1 via window.top
    # Full page reload starts a fresh session — user lands on login screen.
    ...
    # brand_bar: st.markdown HTML in col1 + real st.button in col2
    # col2 CSS gives it the same dark-teal look so both appear as ONE bar
    brand_bar(user)
