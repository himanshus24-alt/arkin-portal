...
def build_users():
    # NOTE: NOT cached — load_data() is already cached; nesting @st.cache_data
    # inside another @st.cache_data raises an error in Streamlit ≥ 1.26 / Python 3.12+
    org = load_data()["org"]
    u = {}
    # RMs
...
def incentive_grid_dict():
    # NOT cached — load_data() is already cached; avoid nesting @st.cache_data
    g = load_data()["grid"]
    GRID = {}
...
def main():
    ...
    # ---- pre-flight: verify the data file and sheet names before anything else ----
    if not DATA_FILE.exists():
        st.error(
            f"**Data file not found.**\n\n"
            f"Expected: `arkin_dummy_data.xlsx` next to `arkin_app.py` in your repo.\n\n"
            f"Looking at: `{DATA_FILE}`"
        )
        st.stop()
    try:
        ...
        _wb.close()
        # 1) sheet-level check
        _need = {"Performance", "Profitability", "Portfolio", "Org Hierarchy",
                 "Attrition", "Announcements", "Incentive Grid"}
        _missing = _need - set(_found)
        if _missing:
            st.error(
                f"**Wrong data file — looks like the old v1 workbook.**\n\n"
                f"Missing sheets: `{', '.join(sorted(_missing))}`\n\n"
                f"Found sheets: `{', '.join(_found)}`\n\n"
                f"➡ Replace `arkin_dummy_data.xlsx` in your repo with the **v2 file** "
                f"(7 sheets), then in Streamlit Cloud go to Manage App → Reboot."
            )
            st.stop()
        # 2) column-level check on Org Hierarchy (catches v1 file with wrong column names)
        _org_df = pd.read_excel(DATA_FILE, sheet_name="Org Hierarchy", nrows=1)
        _org_cols = set(_org_df.columns.tolist())
        _need_cols = {"RM ID", "RM Name", "BBM ID", "BBM Name",
                      "ABM ID", "ABM Name", "RBM ID", "RBM Name",
                      "ZBM ID", "ZBM Name", "Branch Name"}
        _missing_cols = _need_cols - _org_cols
        if _missing_cols:
            st.error(
                f"**Wrong data file — Org Hierarchy sheet has unexpected columns.**\n\n"
                f"Missing columns: `{', '.join(sorted(_missing_cols))}`\n\n"
                f"Columns found: `{', '.join(sorted(_org_cols))}`\n\n"
                f"➡ This is likely the **v1 Excel file**. Replace it with the v2 "
                f"`arkin_dummy_data.xlsx` we generated, then Reboot from Manage App."
            )
            st.stop()
    except Exception as _e:
        st.error(f"Could not open data file: `{_e}`\n\nFile path: `{DATA_FILE}`")
        st.stop()
    # ---- end pre-flight ----
